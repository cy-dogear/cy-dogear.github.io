// postdetail.js
// Dot2Dot 广播详情页 / 全部历史页 - 从 JSONBin 读取数据，只负责动态内容填充
...
let allUsers = []
let allMessages = []
let currentUserId = null      // 当前页面的用户（广播的作者或查看历史的用户）
...
function bindNavLinks() {
    ...
    if (postNav) {
        postNav.removeEventListener('click', handleNavClick)
        postNav.addEventListener('click', handleNavClick)
    }
    
    if (mypostNav) {
        mypostNav.removeEventListener('click', handleNavClick)
        mypostNav.addEventListener('click', handleNavClick)
    }
...
async function init() {
    try {
        ...
        // 如果没有 user 但有 msg，从 msg 获取 user_id
